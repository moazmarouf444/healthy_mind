@if(isset($inputs))
    @foreach ($inputs as $inputName => $input)
            ...
            <div class="form-group">
                ...
                {{--  <div class="controls">
                    <input
                        ...
                        placeholder="{{isset($input['placeholder']) ? $input['placeholder'] : awtTrans('قم بإدخال ') . ' ' . $input['text']}}" 
                        
                        {{(isset($input['validation']) && $input['validation'] != false && $input['validation']['type'] == 'required ') ? 'required  data-validation-required-message="' . awtTrans(" هذا الحقل مطلوب ") . '"' : ''}}
                ...
                <div class="col-12 p-0 ">
                    <div class="files_uploader_container  p-2" id="{{$inputName}}_cont">
                        ...
                        @if(in_array(explode('.' ,$file[$input['files']['value']])[1] ,['jpg' ,'JPG' ,'png' ,'PNG' ,'jpeg' ,'JPEG' ,'SVG' ,'svg' ,'TIFF' ,'tiff' ,'webp' ,'WEBP']) || !isset($input['files']['name']))
        ...
        @elseif ($input['input'] == 'seo')
            <h5 class="w-100 pt-5 px-3 pb-2">{{awtTrans('اضافه seo ')}}</h5>
                @foreach (languages() as $lang)
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            <label for="first-name-column">{{__('site.meta_title_'.$lang) }}</label>
                            <div class="controls">
                                <textarea disabled class="form-control" placeholder="{{__('site.write') . __('site.meta_title_'.$lang)}}" required data-validation-required-message="{{awtTrans('هذا الحقل مطلوب')}}" cols="30" rows="10">{{$item->getTranslations('meta_title')[$lang] ?? ''}}</textarea>
                            </div>
                        </div>
                    </div>
                @endforeach
                @foreach (languages() as $lang)
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            ...
                            <div class="controls">
                                <textarea disabled class="form-control" placeholder="{{__('site.write') . __('site.meta_description_'.$lang)}}" required data-validation-required-message="{{awtTrans('هذا الحقل مطلوب')}}" cols="30" rows="10">{{$item->getTranslations('meta_description')[$lang] ?? ''}}</textarea>
                            </div>
                        </div>
                    </div>
                @endforeach
                @foreach (languages() as $lang)
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            ...
                            <div class="controls">
                                <textarea disabled class="form-control" placeholder="{{__('site.write') . __('site.meta_keywords_'.$lang)}}" required data-validation-required-message="{{awtTrans('هذا الحقل مطلوب')}}" cols="30" rows="10">{{$item->getTranslations('meta_keywords')[$lang] ?? ''}}</textarea>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="col-md-6  col-12 {{isset($input['col_md']) ? ' col-md-' . $input['col_md'] : ' '}}">
                    <div>
                        <div class="form-group {{$input['map_address'] === null ? 'd-none' : ''}}">
                            <label for="commercial_number">{{awtTrans('العنوان')}}</label>
                            <div class="controls">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="mapSearch" class="form-control" placeholder="{{awtTrans('البحث على الخريطة')}}" >
